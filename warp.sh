...
Init_WARP_Client() {
    ...
    if [[ $(warp-cli status) = *Missing* ]]; then
...
Set_WARP_Protocol_Masque() {
    Init_WARP_Client
    log INFO "Setting WARP tunnel protocol to MASQUE..."
    warp-cli tunnel protocol set MASQUE
    log INFO "Protocol set. Reconnect to apply changes."
    warp-cli status
}
